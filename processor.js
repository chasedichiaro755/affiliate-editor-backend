...
async function processVideo(inputPath, settings, outputDir) {
  ...
  const rawSegments = words.map(w => {
    ...
    // Safety: lead-in/out can never eat more than 20% of any word
    const safeLeadIn = leadIn < 0 ? Math.max(leadIn, -(wordDuration * 0.2)) : leadIn;
    const safeLeadOut = leadOut < 0 ? Math.max(leadOut, -(wordDuration * 0.2)) : leadOut;
  ...
  const result = { 
    outputPath, 
    retakeSuggestions,
    segments: merged.map((s, i) => ({ index: i, start: +s.start.toFixed(3), end: +s.end.toFixed(3) }))
  };
  return result;
}
