...
async function processVideo(inputPath, settings, outputDir) {
  ...
  // STEP 5: Detect retakes if enabled
  let retakeRangesToRemove = [];
  if (removeRetakesEnabled) {
    console.log('Checking for retakes...');
    const retakes = removeRetakes(words, duration);
    if (retakes) retakeRangesToRemove = retakes;
  }

  // STEP 6: Build speech segments with safety checks
  const MIN_SEGMENT = 0.3; // never shorter than 0.3s
  const rawSegments = words.map(w => {
    const wordStart = w.start / 1000;
    const wordEnd = w.end / 1000;
    const wordDuration = wordEnd - wordStart;

    // Safety: lead-in can never eat more than 50% of the word
    const safeLeadIn = Math.min(leadIn, wordDuration * 0.5);
    // Safety: lead-out can never eat more than 50% of the word
    const safeLeadOut = Math.min(leadOut, wordDuration * 0.5);
  ...
  // STEP 7: Remove retake segments from merged
  if (retakeRangesToRemove.length > 0) {
    merged = merged.filter(seg => {
      for (const retake of retakeRangesToRemove) {
        // If segment overlaps with a retake range, remove it
        if (seg.start < retake.end && seg.end > retake.start) return false;
      }
      return true;
    });
    console.log(`After retake removal: ${merged.length} segments`);
  }
