...
async function processVideo(inputPath, settings, outputDir) {
  ...
  const ffmpegCmd = `ffmpeg -i "${inputPath}" -filter_complex "${filterParts.join(';')}" -map "[vout]" -map "[aout]" -c:v libx264 -preset ultrafast -crf 28 -vf scale=1080:1920 -c:a aac -threads 1 -y "${outputPath}"`;
